...
public static class EventHandlers
{
    ...
    public static async Task HandleMessageCreated(DiscordClient client, MessageCreateEventArgs e)
    {
        if (e.Author.Id != client.CurrentApplication.Id && // don't reply to yourself
            (Bot.Instance.Database.Channels.IsJoined(e.Channel.Id) || 
             (Bot.Instance.Database.Users.IsFollowing(e.Author.Id))))
        {
    ...
    public static async Task HandleReactionAdded(DiscordClient client, MessageReactionAddEventArgs e)
    {
        if (e.Emoji.Name == Bot.Icon)
        {
            ...
            {
                if (!Bot.Instance.Database.Users.IsFollowing(e.User.Id))
                {
                    Bot.Instance.Database.Users.Follow(e.User.Id);
    ...
    private static async Task<bool> CheckMessage(DiscordMessage message)
    {
        ...
        if (message.Content.Count(char.IsLetter) < Bot.Instance.MinLetterLimit) return true;

        words.UnionWith(Utils.GetWordsInString(content));

        if (Bot.Instance.Database.Words.Contains(words)) return false;
        
        Bot.Instance.Database.Users.IncreaseMessageCount(message.Author.Id);
